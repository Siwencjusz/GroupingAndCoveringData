...
namespace BusinessLogicLayer.Matrix
{
    public class CoverMatrixManager : ICoverMatrixManager
    {
        ...
        public async Task<Result<CoverSampleResult>> GetMatrix(FileData fileData,
            double low,
            double high,
            ...
            ProgressBarModel progressBarModel)
        {
            ...
            var matrices = (await ComputeForEvery2(fileData, groupMethod, paramInput, highs, lows, progressBarModel)).OrderByDescending(y => y.Grade).ToList();
            ...
            var result = new CoverSampleResult(max, matrices);
            ...
            ProgressBarModel progressBarModel)
        {
            try
            {
                ...
                var listOfGroupsOfDataObjects =
                     groupMethod.GroupElementsBy(
                        fileData.DataObjects,
                        fileData.Attributes,
                        paramInput)
                         .ToArray();

                progressBarModel.Progress = 30;

                CountPositiveDecisions(fileData, listOfGroupsOfDataObjects);

                var max = await Compute(fileData, low, high, listOfGroupsOfDataObjects, progressBarModel, 0);

                var matrixRows = _coverMatrixClassificator.Classify(fileData.TestObjects, max.AttributesCovers, max.HighZeroColumns, max.LowZeroColumns);
                max.TestMatrix = new Core.Common.Items.MatrixFeatures.Matrix(matrixRows);

                progressBarModel.Progress = 95;

                var dataMatrix = _matrixToGridMatrix.TransformToDataTable(max.DataMatrix);
                var testMatrix = _matrixToGridMatrix.TransformToDataTable(max.TestMatrix);

                var result = new CoverSampleResult(max, new [] { max });
            ...
            {
                return new Result<CoverSampleResult>(e);
            }
        }
        ...
        private async Task<IEnumerable<CoverResult>> ComputeForEvery2(
            FileData fileData,
            IGroupingMethod groupMethod,
            double paramInput,
            double[] highs,
        ...
        private IEnumerable<Task<CoverResult>> GetCoverTasks(FileData fileData, ProgressBarModel progressBarModel, (double, double)[] all,
            AttributeGroupsOfObjects[] listOfGroupsOfDataObjects, double step)
        {
            foreach (var pair in all.AsParallel())
            {
                yield return
        ...
        private static IEnumerable<(double, double)> GetLowsAndHighs(
            ...
            double step)
        {
            while (low >= 0 && low <= 1 && high >= 1 && high <= 2)
            {
                low += step;
                high -= step;
        ...
        private async Task<CoverResult> Compute(
            FileData fileData,
            double LOW,
            double HIGH,
            AttributeGroupsOfObjects[] listOfGroupsOfDataObjects,
            ...
            double step)
        {
            ...
            result.DataMatrix = _coverMatrixGenerator.CreateMatrix(fileData.DataObjects, LOW, HIGH, result.AttributesCovers, result.LowZeroColumns, result.HighZeroColumns);
