...
namespace Models.ViewModels
{
    ...
    public class MainViewModel : BaseNotifyError
    {
        ...
        public MainViewModel(
            ...
            ITxtExporter txtExporter) : base(validateService)
        {
            ...
            CmdComputeData = new DelegateCommand(ComputeData, RunEnableCheck);
            CmdReadFile = new DelegateCommand(ReadFile, () => RunButtonIndicatorVisible);
            CmdExportToExcel = new DelegateCommand(ExportToExcel, () => CanEnable);
            CmdExportTstToTxt = new DelegateCommand(() => ExportToTxt(CoverResult.TestMatrix, "_Obiekty_testowe"), () => CanEnable);
            CmdExportTrnToTxt = new DelegateCommand(() => ExportToTxt(CoverResult.DataMatrix, "_Obiekty_treningowe"), () => CanEnable);
        ...
        private void ExportToTxt(Matrix coverResultDataObjects, string suffix)
        {
            ExcelIndicatorVisible = true;
            Thread.CurrentThread.IsBackground = true;
            var result = _txtExportert.ExportToTxt(coverResultDataObjects, suffix);
        ...
        private void ExportToExcel()
        {
            ...
            var result = _excelWriter.ExportToExcel(FileData.FileName, CoverSample);
        ...
        {
            var result = _fileReaderProvider.ReadFile(DataFilter);
            if (!result.HasValue())
            {
                if (result.HasErrors())
                {
            ...
            RawDataString = FileData.JoinedSring;
            ...
            var x = Task.Factory.StartNew(async () =>
                     {
                         try
                         {
                             ...
                             Result<CoverSampleResult> result = null;
                             if (!UseStep)
                             {

                                 result = await
                                     _coverMatrixManager.GetMatrix(FileData, low, high, method,
                                         paramInput, Progress);
                             }
                             else
                             {
                             ...
                             CoverSample.SHIGH = high;
                             CoverSample.SLOW = low;
                             CoverSample.STEP = step;
                             CoverSample.SelecteMethod = method.MethodName;
                             CoverSample.SelecteMethodParam = paramInput;
        ...
        public Matrix CoverMatrix
        {
            ...
            {
                _coverMatrix = value;
                CoverMatrixDataTable = _coverMatrix.DataTable;
